...
### Full Waypoint Table — All Tokens (Locked May 9, 2026)

Version A (Light to Light) confirmed. All intermediate colors hand-picked to avoid muddy or desaturated midpoints. Linear RGB interpolation is NOT used. Interpolate smoothly between adjacent waypoints only.
...
Morning transition (8:00 AM to 9:00 AM) uses the same table in reverse: 8:00 AM = 6:00 PM column, 8:15 AM = 5:45 PM column, etc.

Dark background variant (Version B) was evaluated and deferred. Not part of the initial build. May revisit post-launch.
...
| SessionTypeCards | Services | Hover reveals dark gradient caption overlay. Image scales 1.03. Locked to Services only (May 8). Home page keeps hero + testimonials without competing hover interactions. |
...
- [x] SessionTypeCards placement: Services only (locked May 8)
